...
# -----------------------------
DATASET_NAME = "clamav"  # set to: clamav | curl | nmap | openssl | unrar
...
# Optional: manual skip by exact filename (no path)
MANUAL_SKIP = set([
    # Example:
    # "x64-gcc-4.8-O2_libclamav.so.9.0.0",
])

# Strong recommendation: skip shared libraries to avoid huge files and frequent segfaults
SKIP_SHARED_LIBS = True  # if True, skips names containing ".so"

# Progress log (helps identify last processed binary if a segfault happens)
...
def append_crash(name: str, path: str) -> None:
    os.makedirs(os.path.dirname(CRASH_LIST_PATH), exist_ok=True)
    with open(CRASH_LIST_PATH, "a", encoding="utf-8") as fp:
        fp.write(f"{name}\t{path}\n")


# -----------------------------
# Normalization (PalmTree-style)
# -----------------------------
...
def build_string_map(proj: angr.Project, min_len: int = 4) -> Dict[int, str]:
    """
    Best-effort ASCII string extraction from readable sections.
    """
...
def build_function_dfg(proj: angr.Project, func_addr: int) -> Optional[nx.DiGraph]:
    ...
    # Collect instruction addresses per basic block (used for fallback edges)
    ...
    # --- Attempt ReachingDefinitions (best-effort) ---
    rd_ok = True
    try:
    ...
    # --- Fallback if RD failed or produced no edges ---
    if (not rd_ok) or (len(edges) == 0):
        ...
        # connect blocks via function CFG edges (best-effort)
        try:
    ...
    # Connect entry node to nodes with no incoming edges
...
def main():
    ...
    for name in os.listdir(in_dir):
        ...
        if SKIP_SHARED_LIBS and ".so" in name:
            continue
    ...
    for name, bp in tqdm(binaries, desc=f"DFG({DATASET_NAME})"):
        ...
        # Write marker BEFORE processing (survives segfaults)
