...
def write_pairs_for_binary(binary_path: Path, out_path: Path, max_funcs=None):
    ...
    try:
        ...
        # IMPORTANT FIX:
        # Disable syscall + indirect jump resolution (Z3 triggers crashes otherwise)
        ...
        with out_path.open("w") as f:
            ...
            for func in cfg.kb.functions.values():
                ...
                for i in range(len(instrs) - 1):
                    f.write(f"{instrs[i]}\t{instrs[i+1]}\n")

        return True

    except Exception as e:
        print(f"[!] Skipping binary due to error: {binary_path}")
...
def main():
    ...
    # OUTPUT FOLDER
    out_root = repo_root / "evaluation" / project
    ...
    for bin_path in sorted(binaries):
        ...
        if write_pairs_for_binary(bin_path, out_path):
            ok += 1
        else:
            fail += 1
    ...
    print(f"[+] Summary: success={ok}, failed={fail}")
